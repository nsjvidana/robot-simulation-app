...
impl IntoXurdf for urdf_rs::Pose {
    ...
    fn into_xurdf(self) -> Self::Target {
        xurdf::Pose {
            rpy: Vector3::from_data(ArrayStorage([self.rpy.0])),
            xyz: Vector3::from_data(ArrayStorage([self.xyz.0])),
        }
    }
}
    ...
    fn into_xurdf(self) -> Self::Target {
        ...
        match self {
            Self::Box { size } => Geometry::Box {
                size: Vector::from_data(ArrayStorage([size.0])),
            },
            ...
            Self::Mesh { filename, scale } => Geometry::Mesh {
                filename,
                scale: scale.map(|s| Vector::from_data(ArrayStorage([s.0]))),
            },
            _ => unimplemented!(),
        }
    }
...
pub fn urdf_rs_robot_to_xurdf(robot: urdf_rs::Robot) -> xurdf::Robot {
    ...
    for j in robot.joints.into_iter() {
        joints.push(xurdf::Joint {
            ...
            child: j.child.link,
            axis: Vector::from_data(ArrayStorage([j.axis.xyz.0])),
